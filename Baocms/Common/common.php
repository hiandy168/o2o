...
function setUid($uid){
    ...
    cookie('HJL_TOKEN',$uid,86400*365); //存一年
    return true;
}
function clearUid(){
    cookie('HJL_TOKEN',null); //存一年
    return true;
}

function getUid(){
    import("ORG/Crypt/Base64");
    $token = cookie('HJL_TOKEN');
...
//通过百度API获得当前IP地址
function get_current_address_by_baidu($ip=""){
    if(!$ip){
         $ip = get_client_ip(); 
    }
    $content = file_get_contents("http://api.map.baidu.com/location/ip?ak=K6NUCfSkWYRoWgpp7ssdfPc2&ip={$ip}");
    return json_decode($content);
	//return array();
}
//获得拼音
function get_pinyin($_String, $_Code = 'utf-8',$isInitial = FALSE){
    //引入拼音类
    import("ORG.Util.Pinyin");
    $pinyin = new Pinyin();
    return $pinyin->run($_String, $_Code,$isInitial);
