...
class ChaoshicommentAction extends CommonAction {
    ...
    public function index() {
        $Chaoshicomment = D('Chaoshicomment');
        ...
        // 保存搜索信息到cookie中
        if(cookie(md5('ChaoshiCommentSearchIndexMessage'))){
            $map = cookie(md5('ChaoshiCommentSearchIndexMessage'));
            if(cookie(md5('ChaoshiCommentSearchIndexMessageComm'))){
                $map['contents'] = cookie(md5('ChaoshiCommentSearchIndexMessageComm'));
            }
        }
        ...
        if($_SERVER['REQUEST_METHOD'] == 'GET') {
            ...
            if($map['store_id'] == true){
                $store = M('Chaoshi')->find($map['store_id']);
                $this->assign('store_name', $store['store_name']);
        ...
        cookie(md5('ChaoshiCommentSearchIndexMessage'), $map, 900);
        cookie(md5('ChaoshiCommentSearchIndexMessageComm'), $map['contents'], 900);

        // 分页
        $Page = new Page($Chaoshicomment->where($map)->count(), 25); // 实例化分页类 传入总记录数和每页显示的记录数
    ...
    private function getApply($comment_id, $closed, $words){
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if (is_array($comment_id)) {
                $obj = D('Chaoshicomment');
                foreach ($comment_id as $id) {
                    $obj->save(array('comment_id' => $id, 'audit' => $closed));
                }
                $this->baoSuccess($words, U('chaoshicomment/index'));
            }else {
