...
class ElecateshopAction extends CommonAction {

   public function _initialize() {
        $this->elecate_mod =D('EleCateShop');
        parent::_initialize();
    }
    ...
    public function index(){
        ...
        $count = $this->elecate_mod->where($map)->count(); // 查询满足要求的总记录数
        $Page = new Page($count, 25); // 实例化分页类 传入总记录数和每页显示的记录数
        $show = $Page->show(); // 分页显示输出

        $list = $this->elecate_mod
            ->where($map)
...
//        var_dump($list);
    ...
    public function checkcatename(){
        ...
        $status = D('EleCateShop')->where($data)->count();
        if($status){
    ...
    public function edit($cate_id=0){
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($rst) {
                $this->baoSuccess('操作成功', U('elecateshop/index'));
            }
            $this->baoError('操作失败');
        }
        if(is_numeric($cate_id) && ($cate_id = (int) $cate_id)){
            if (!$detail = $eleModel->find($cate_id)) {
    ...
    public function delete($cate_ids = 0) {
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            foreach ($cate_ids as $cate_id) {
                if(!$eleModel->where(array('cate_id' => $cate_id))->delete()){
                    $this->baoError('含不存在的分类');
                };
            }
            $this->baoSuccess('删除成功！', U('elecateshop/index'));
        }
        if (is_numeric($cate_ids) && ($cate_id = (int) $cate_ids)) {
            $findCate = $eleModel->find($cate_id);
            if(!$findCate){
