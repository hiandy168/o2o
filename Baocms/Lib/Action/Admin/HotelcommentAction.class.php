...
class HotelcommentAction extends CommonAction {
    ...
    public function index() {
        ...
        // 保存搜索信息到cookie中
        if(cookie(md5('HotelCommentSearchIndexMessage'))){
            $map = cookie(md5('HotelCommentSearchIndexMessage'));
            if(cookie(md5('HotelCommentSearchIndexMessageComm'))){
                $map['contents'] = cookie(md5('HotelCommentSearchIndexMessageComm'));
            }
        }
        ...
        cookie(md5('HotelCommentSearchIndexMessage'), $map, 900);
        cookie(md5('HotelCommentSearchIndexMessageComm'), $map['contents'], 900);

        // 分页
        $Page = new Page($Hotelcomment->where($map)->count(), 25); // 实例化分页类 传入总记录数和每页显示的记录数
    ...
    private function getApply($comment_id, $closed, $words){
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if (is_array($comment_id)) {
                ...
                $this->baoSuccess($words, U('hotelcomment/index'));
            }else {
