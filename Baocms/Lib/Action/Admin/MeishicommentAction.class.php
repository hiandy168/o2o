...
class MeishicommentAction extends CommonAction {
    ...
    public function index() {
        $Meishicomment = D('Meishi_comment');
        ...
        // 保存搜索信息到cookie中
        if(cookie(md5('MeishiCommentSearchIndexMessage'))){
            $map = cookie(md5('MeishiCommentSearchIndexMessage'));
            if(cookie(md5('MeishiCommentSearchIndexMessageComm'))){
                $map['contents'] = cookie(md5('MeishiCommentSearchIndexMessageComm'));
            }
        }
        ...
        if($_SERVER['REQUEST_METHOD'] == 'GET') {
            ...
            if($map['store_id'] == true){
                $store = M('Meishi')->find($map['store_id']);
                $this->assign('store_name', $store['store_name']);
        ...
        cookie(md5('MeishiCommentSearchIndexMessage'), $map, 900);
        cookie(md5('MeishiCommentSearchIndexMessageComm'), $map['contents'], 900);
//        var_dump($map);

        // 分页
        $Page = new Page($Meishicomment->where($map)->count(), 15); // 实例化分页类 传入总记录数和每页显示的记录数
...
//        var_dump($list[0]);
    ...
    private function getApply($comment_id, $closed, $words){
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if (is_array($comment_id)) {
                $obj = D('Meishi_comment');
                foreach ($comment_id as $id) {
                    $obj->save(array('comment_id' => $id, 'audit' => $closed));
                }
                $this->baoSuccess($words, U('meishicomment/index'));
            }else {
    ...
    public function initialIndex(){
        cookie(md5('MeishiCommentSearchIndexMessage'), NULL, 0);
        $this->baoSuccess('初始化成功', U('meishicomment/index'), 1000);
    }
