    ...
    public function audit($cash_id = 0, $status = 0)
    {
        ...
        if (is_numeric($cash_id) && ($cash_id = (int)$cash_id)) {
            ...
            if ($data['status'] == 0) {
                ...
                //微信通知
                $userInfo = D('Users')->where(array('user_id' => $data['user_id']))->find();
                D('Sms')->sendSms('money_cash_no', $userInfo['mobile'], array(
                    'money' => $data['money']
                ));
            ...
            foreach ($cash_id as $id) {
                ...
                //如果是拒绝则返还钱
                $userInfo = D('Users')->where(array('user_id' => $data['user_id']))->find();
                D('Sms')->sendSms('money_cash_ok', $userInfo['mobile'], array(
                    'money' => $data['money']
                ));
            }
            $this->baoSuccess('操作成功！', U('usercash/index'));
