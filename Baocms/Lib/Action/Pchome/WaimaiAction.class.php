...
class WaimaiAction extends CommonAction {
    ...
    public function order() {
        ...
        if ($shop['is_new'] && !D('Eleorder')->checkIsNew($this->uid, $shop_id)) { //如果是新单  
            if ($total['money'] >= $shop['full_money']) { //满足新单的条件 立马减几块钱
                $num1 = (int) (($total['money'] - $shop['full_money']) / 10); //10块钱加1规则
    ...
    public function pay2() {
        ...
        if ($code == 'wait') { //如果是货到付款
           ...
           $_data_order = array(//整体变更
            ...
            $this->ajaxReturn(array('status' => 'success', 'msg' => '选择支付方式成功！下面请进行支付！', 'url' => U('payment/payment', array('log_id' => $logs['log_id'],'type'=>'ele'))));
        }
    }
...
//         }
    ...
    public function index() {
        ...
        switch ($price) {
            case 1:
                $map['since_money'] = array('ELT', '50');
                break;
            case 2:
                $map['since_money'] = array('between', '50.01,100');
                break;
            case 3:
                $map['since_money'] = array('between', '100.01,200');
                break;
            case 4:
                $map['since_money'] = array('EGT', '200.01');
                break;
        }
        $linkArr['price'] = $price;
