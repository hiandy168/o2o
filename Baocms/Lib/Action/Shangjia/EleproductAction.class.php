...
class EleproductAction extends CommonAction {
	...
	public function _initialize() {
		...
		$this->elecates = D('Elecate')->where(array('store_id'=>$this->store_id,'closed'=>0))->select();
	...
	private function createCheck() {
		...
		$data['settlement_price'] = (int)( $data['price'] - ($data['price'] *  $this->ele['rate'] /10));
	...
	public function delete($product_id = 0) {
		if (is_numeric($product_id) && ($product_id = (int) $product_id)) {
			$obj = D('Eleproduct');
			if(!$detail = $obj->where(array('store_id'=>$this->store_id,'product_id'=>$product_id))->find()){
