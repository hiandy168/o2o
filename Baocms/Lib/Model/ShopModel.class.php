...
class ShopModel extends CommonModel{
    ...
    protected $_validate=  array(
        array('shop_name','require','商家名必填'),
        array('shop_name','','商家名已存在',0,'unique',0),
     );
	//删除标示
    public $flag = array(
    						'exist' => 0,
    ...
    //商家模块名
    private $modelName = array('Ele', 'Meishi', 'Chaoshi', 'Hotel');
    ...
    {
    	foreach ($this->modelName as $v) {
    		$model = D($v);
    		$storeIds = $model->getStoreIds($shopId);
    		if ($storeIds) {
    			foreach ($storeIds as $storeId) {
    				if (!$model->deleteAll($storeId['store_id'])) {
    					return false;
    				}
    			}
    		}
    	}
    	$this->deleteById($shopId);
    	return true;
    }
    ...
    {
    	return $this->where(array('shop_id' => $shopId))->save(array('closed' => $this->flag['delete']));
    }
}