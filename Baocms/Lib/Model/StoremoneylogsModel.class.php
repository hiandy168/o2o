...
class StoremoneylogsModel extends CommonModel{
    protected $pk   = 'log_id';
    protected $tableName =  'store_money_logs';
    ...
    public function tjmonthCount($month = '',$shop_id = '',$store_id = 0,$type=0){
        $sql = "";
        $month = $month ? str_replace('-', '', $month): '';
        $shop_id = (int)$shop_id;
        if($store_id){
            $store_limit = " and store_id in ($store_id) and store_type = $type";
            $store_limit_where = " where store_id in ($store_id) and store_type =$type";
        }
        if($month && $shop_id){
            $sql="select count(1) as num from (SELECT sum(money) as money,FROM_UNIXTIME(create_time,'%Y%m') as m,shop_id  FROM ".$this->getTableName()." where FROM_UNIXTIME(create_time,'%Y%m') = '{$month}' $store_limit  and shop_id='{$shop_id}' group  by  FROM_UNIXTIME(create_time,'%Y%m'),shop_id)tb  ";
        }else{
            if($month){
                $sql=" select count(1) as num from (SELECT sum(money) as money,FROM_UNIXTIME(create_time,'%Y%m') as m,shop_id  FROM ".$this->getTableName()." where FROM_UNIXTIME(create_time,'%Y%m') = '{$month}' $store_limit group  by  FROM_UNIXTIME(create_time,'%Y%m'),shop_id)tb  ";

            }elseif($shop_id){
                $sql=" select count(1) as num from (SELECT sum(money) as money,FROM_UNIXTIME(create_time,'%Y%m')  as m,shop_id  FROM ".$this->getTableName()." where shop_id='{$shop_id}' $store_limit   group  by  FROM_UNIXTIME(create_time,'%Y%m'),shop_id)tb  ";
            }else{
                $sql=" select count(1) as num from (SELECT sum(money) as money,FROM_UNIXTIME(create_time,'%Y%m')  as m,shop_id  FROM ".$this->getTableName()."  $store_limit_where  group  by  FROM_UNIXTIME(create_time,'%Y%m'),shop_id)tb  ";
            }
        }
    ...
    public function tjmonth($month = '',$shop_id = '',$start,$num,$store_id = 0,$type=0){
        $sql = "";
        $month = $month ? str_replace('-', '', $month): '';
        $start = (int)$start;
        $num = (int)$num;
        ...
        if($month && $shop_id){
            $sql="SELECT sum(money) as money,FROM_UNIXTIME(create_time,'%Y%m') as m,shop_id  FROM ".$this->getTableName()." where FROM_UNIXTIME(create_time,'%Y%m') = '{$month}' and shop_id='{$shop_id}' $store_limit group  by  FROM_UNIXTIME(create_time,'%Y%m'),shop_id order by FROM_UNIXTIME(create_time,'%Y%m') desc  limit {$start},{$num} ";
        }else{
            if($month){
                $sql="SELECT sum(money) as money,FROM_UNIXTIME(create_time,'%Y%m') as m,shop_id  FROM ".$this->getTableName()." where FROM_UNIXTIME(create_time,'%Y%m') = '{$month}'  $store_limit group  by  FROM_UNIXTIME(create_time,'%Y%m'),shop_id  order by FROM_UNIXTIME(create_time,'%Y%m') desc   limit {$start},{$num}  ";

            }elseif($shop_id){
                $sql="SELECT sum(money) as money,FROM_UNIXTIME(create_time,'%Y%m')  as m,shop_id  FROM ".$this->getTableName()." where shop_id='{$shop_id}'  $store_limit  group  by  FROM_UNIXTIME(create_time,'%Y%m'),shop_id  order by FROM_UNIXTIME(create_time,'%Y%m') desc   limit {$start},{$num} ";
            }else{
                $sql="SELECT sum(money) as money,FROM_UNIXTIME(create_time,'%Y%m')  as m,shop_id  FROM ".$this->getTableName()."  $store_limit_where  group  by  FROM_UNIXTIME(create_time,'%Y%m'),shop_id  order by FROM_UNIXTIME(create_time,'%Y%m') desc  limit {$start},{$num}  ";
            }
        }
        //echo $sql;
        $data = $this->query($sql);
    ...
    public function money($bg_time,$end_time,$shop_id,$store_id = 0,$type=0){
        $bg_time   = (int)$bg_time;
        ...
        if(!empty($shop_id)){
            $data = $this->query(" SELECT sum(money) as price,FROM_UNIXTIME(create_time,'%m%d') as d from  ".$this->getTableName()."   where  create_time >= '{$bg_time}' AND create_time <= '{$end_time}' $store_limit AND shop_id = '{$shop_id}'  group by  FROM_UNIXTIME(create_time,'%m%d')");
        }else{
            $data = $this->query(" SELECT sum(money) as price,FROM_UNIXTIME(create_time,'%m%d') as d from  ".$this->getTableName()."   where  create_time >= '{$bg_time}' AND create_time <= '{$end_time}'  $store_limit group by  FROM_UNIXTIME(create_time,'%m%d')");
        }
        $showdata = array();
        $days = array();
