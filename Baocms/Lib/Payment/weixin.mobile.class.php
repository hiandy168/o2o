...
/*     public function getCode($logs, $payment)
    {
        ...
        $input->SetNotify_url(__HOST__ . U('mobile/payment/respond', array(
            'code' => 'weixin'
        )));
            		...
            		WeixinJSBridge.invoke(
            			...
            			function(res){
                                        if(res.err_msg ==\'get_brand_wcpay_request:ok\'){ 
                                            location.href="' . U('mobile/payment/yes', array(
                        'log_id' => $logs['logs_id']
                    )) . '";
                                        }
            			}
    ...
    public function Queryorder($transaction_id)
    {
        ...
        /* 检查支付的金额是否相符 ，微信金额单位为分*/
        if (!D('Payment')->checkMoney(think_decrypt($result['out_trade_no']), $result['total_fee']/100)) {
            return false;
        }
    ...
    public function respond() {   
        ...
        D('Payment')->logsPaid(think_decrypt($result['out_trade_no']));
        return true;
    }
