...
class  ThinkTemplate {
    ...
    protected function parseExtend($content) {
        ...
        if($find) {
            ...
            preg_replace_callback('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/is', function($matches) use($that) {$that->parseBlock($matches[1],$matches[2]);}, $content);
            // 读取继承模板
            $array      =   $this->parseXmlAttrs($matches[1]);
            ...
            $content    =   preg_replace_callback('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/is', function($matches) use($that) {$that->replaceBlock($matches[1],$matches[2]);}, $content);
        }else{
            //$content    =   preg_replace('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/eis',"stripslashes('\\2')",$content);  
            $content    =   preg_replace_callback('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/is', function($matches) use($that) {$that->stripslashes($matches[2]);}, $content);          
        }
        return $content;
    }
